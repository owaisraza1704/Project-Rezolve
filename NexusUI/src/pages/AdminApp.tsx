...
import { Activity, LogOut, Settings, Shield, Trash2, UserCheck, Users } from 'lucide-react'
    ...
    <div className="flex min-h-screen bg-slate-100 font-sans">
      <aside className="flex w-64 flex-col border-r border-slate-300 bg-slate-900 text-slate-300">
        ...
        <div className="flex-1 space-y-1 p-4">
          ...
          <NavItem active={view === 'users'} icon={Users} label="User Management" onClick={() => setView('users')} />
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      <div className="mb-8">
        ...
        <p className="mt-1 text-slate-600">Manage registered users and their platform access.</p>
      </div>
            ...
            {!isLoading && !errorMessage
              ? users.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4 text-slate-600">{formatJoinDate(user.created_at)}</td>
                    <td className="px-6 py-4 text-right">
                      <button
                        type="button"
