...
const userState = {
  ...
  history: [
    { id: 'TKT-841', title: 'Cannot access billing dashboard', status: 'queued', resolver: null, time: '2h ago' },
    { id: 'TKT-750', title: 'Need help configuring SSO', status: 'resolved', resolver: 'Mike T.', time: '1d ago' },
    { id: 'TKT-612', title: 'Route 53 DNS propagation issues', status: 'closed', resolver: 'Emma W.', time: '5d ago' },
  ],
}
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans">
      <aside className="flex w-64 shrink-0 flex-col border-r border-slate-200/60 bg-white">
        <div className="flex h-16 items-center border-b border-slate-200/60 px-6">
          <span className="font-bold tracking-tight text-slate-900">Nexus User</span>
        </div>
        <div className="flex-1 space-y-1 p-4">
          <NavItem active={view === 'dashboard'} icon={History} label="Request History" onClick={() => setView('dashboard')} />
          <NavItem
            active={view === 'create'}
      ...
      <main className="flex h-screen flex-1 flex-col overflow-hidden">
        {view === 'dashboard' && <DashboardView onOpenSession={() => setView('session')} />}
        {view === 'create' && <CreateTicketView onBackToSession={() => setView('session')} />}
        {view === 'session' && <SessionView onBack={() => setView('dashboard')} />}
      </main>
    </div>
  )
}
    ...
    <div className="flex-1 overflow-auto bg-slate-50 p-8">
      <div className="mx-auto max-w-4xl">
        <header className="mb-8">
          ...
          <p className="mt-1 text-slate-500">Manage your active sessions and past resolutions.</p>
        </header>

        {userState.hasActiveTicket && (
          <section className="mb-10">
            <h2 className="mb-3 text-sm font-bold uppercase tracking-wider text-slate-900">Active Session</h2>
            <Card className="cursor-pointer border-indigo-200 ring-1 ring-indigo-500/10 transition-shadow hover:shadow-md">
              <div className="flex items-center justify-between bg-indigo-50/30 p-5" onClick={onOpenSession}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-3 text-sm text-slate-600">
                      <span className="font-medium text-slate-900">{userState.activeTicket.id}</span>
                      <span>•</span>
                      <span>
                        Assigned to <span className="font-medium text-indigo-700">{userState.activeTicket.resolver}</span>
                      </span>
                      <span>•</span>
        ...
        <section>
          <h2 className="mb-3 text-sm font-bold uppercase tracking-wider text-slate-900">Past & Queued</h2>
          <div className="space-y-3">
            {userState.history.map((ticket) => (
              <Card key={ticket.id} className="transition-shadow hover:shadow-sm">
                <div className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-4">
                    <div
                      className={`flex h-10 w-10 items-center justify-center rounded-full ${
                        ticket.status === 'queued' ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-500'
                      }`}
                    >
                      {ticket.status === 'queued' ? <Clock className="h-5 w-5" /> : <CheckCircle2 className="h-5 w-5" />}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    <Badge variant={ticket.status === 'queued' ? 'warning' : 'neutral'}>
                      {ticket.status === 'queued' ? 'OFFLINE QUEUED' : ticket.status.toUpperCase()}
                    </Badge>
                ...
                {ticket.status === 'queued' && (
                  <div className="flex items-start gap-3 border-t border-amber-100 bg-amber-50/50 px-4 py-3">
                    ...
                    <p className="text-xs leading-relaxed text-amber-800">
                      <strong>Safely queued:</strong> this request missed the live routing window and has moved to the offline queue.
                    </p>
                  </div>
                )}
              </Card>
...
function CreateTicketView({ onBackToSession }: { onBackToSession: () => void }) {
    ...
    return (
      <div className="flex flex-1 items-center justify-center overflow-auto bg-slate-50 p-8">
        <div className="w-full max-w-md rounded-2xl border border-slate-200/60 bg-white p-10 text-center shadow-sm">
          ...
          <h1 className="mb-3 text-2xl font-bold text-slate-900">Active Session in Progress</h1>
          <p className="mb-8 leading-relaxed text-slate-600">
            Users are limited to one active live ticket at a time. Resolve or close the current session before opening another request.
          </p>
    ...
    <div className="relative flex flex-1 flex-col bg-white">
      <header className="z-10 flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-slate-900">{userState.activeTicket.title}</h2>
            </div>
            <p className="mt-0.5 text-xs text-slate-500">
              {userState.activeTicket.id} • Connected to {userState.activeTicket.resolver}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="outline" className="h-9 border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700">
      ...
      <div className="flex-1 overflow-auto bg-slate-50/50 p-6">
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <MessageBubble initials="SJ" tone="agent">
            Hi Alex, I can see the DB timeout spike. Can you confirm the cluster region and recent deploy time?
          </MessageBubble>

          <MessageBubble initials="AL" tone="user">
            We are in us-east-1 and the issue started about 15 minutes after the last deploy.
          </MessageBubble>
        </div>
      </div>
...
function ToolButton({ icon: Icon, label }: { icon: typeof Video; label: string }) {
  return (
    <button className="rounded-md p-2 text-slate-500 transition-all hover:bg-white hover:text-slate-900 hover:shadow-sm" title={label}>
      <Icon className="h-4 w-4" />
    </button>
  )
}
    ...
    <div className={`flex gap-4 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div
        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
          isUser ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200 text-slate-600'
        }`}
      >
        {initials}
      </div>
